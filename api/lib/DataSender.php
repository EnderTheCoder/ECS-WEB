...
class DataSender
{
    ...
    #[NoReturn] public function easyRun($type, $data = null, $msg = null)
    {
//        if (isEmpty($this->code[$type])) $ret = $this->code['dbgMsg'];
//        else $ret = $this->code[$type];
        $ret = empty($this->code[$type]) ? $this->code['dbgMsg'] : $this->code[$type];
        if ($data)
            $ret['data'] = $data;
        else $ret['data'] = array();
        if ($msg) $ret['msg'] = $msg;
    ...
    #[NoReturn] private function jsonReturn($res)
    {
